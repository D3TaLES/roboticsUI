...
try :
    ...
    for idx, step in enumerate(steps) :
        parm = make_ecc_parm(api, cv_parms['current_step'], step.current, idx)
        p_steps.append(parm)
        parm = make_ecc_parm(api, cv_parms['step_duration'], step.duration, idx)
        p_steps.append(parm)
    ...
    # record parameters
    p_record_dt = make_ecc_parm(api, cv_parms['record_dt'], record_dt)
    p_record_dE = make_ecc_parm(api, cv_parms['record_dE'], record_dE)

    # repeating factor
    p_repeat = make_ecc_parm(api, cv_parms['repeat'], repeat_count)
